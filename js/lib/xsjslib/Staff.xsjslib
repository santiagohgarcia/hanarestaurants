...
function BeforeCreate(param) {
	...
	var pStmt = param.connection.prepareStatement(`insert into "${param.afterTableName}" values(?,?,?,?,?)`);
	pStmt.setInteger(1, newObject.StaffId);
...
function CreateStaffToRestaurant(param) {
	var a = param;
}*/